...
import ParkSportsCartItemComponent from '../components/ParkSportsCartItemComponent';
...
export default function () {
  ...
  const handleOrder = () => {
    const destinationScreen = cart.length
      ? 'ParkSportsCartSuccessScreen'
      : 'ParkSportsHomeScreen';
        ...
        <>
          <View style={{height: height * 0.7}}>
            <ScrollView style={styles.flex} contentContainerStyle={styles.main}>
              {cart.map((item, index) => (
                <ParkSportsCartItemComponent item={item} key={index} />
              ))}
            </ScrollView>
          </View>
        </>
